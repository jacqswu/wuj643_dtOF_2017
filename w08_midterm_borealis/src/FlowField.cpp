...
void FlowField::initNoise(float scale)
{
	...
	for (int y = 0; y < nRows; y++) {
		for (int x = 0; x < nCols; x++) {
            ...
            //map and rotate angle based on distance from mouse
            /*
            int dist = ofDist(ofGetMouseX(), ofGetMouseY(), field[index].x, field[index].y);
             
            ofVec2f angleDiff;
            angleDiff.x = ofGetMouseX()-field[index].x;
            angleDiff.y = ofGetMouseY()-field[index].y);
            
            percentage = ofMap(dist, dist, 0, 0, 1);
            if(field[index].x<ofGetMouseX()){
                * percentage
            }
            ...
            */
			...
			noise = ofMap(noise, 0, 1, -PI*1.25, PI*0.25);
